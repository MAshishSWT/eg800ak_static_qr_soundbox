...
#ifdef TX_FWCTRL_ENBLE
void quec_uart_txfwctrl_callback(QL_UART_PORT_NUMBER_E port, char flow_status)
{

	test_log("uart[%d] txfwctrl status:%d \n", port, flow_status);
}
#endif
static void quec_main_uart_test(void * argv)
{
 ...
 ret = ql_uart_open(QL_MAIN_UART_PORT, QL_UART_BAUD_115200, QL_FC_HW);
      ql_uart_register_txfwctrl(QL_MAIN_UART_PORT,quec_uart_txfwctrl_callback);
#endif
