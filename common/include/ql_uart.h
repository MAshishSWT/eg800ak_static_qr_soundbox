...
typedef void (*uart_tx_fwctrl_callback)(QL_UART_PORT_NUMBER_E port, char flow_status);
...
/***********************************************************************
 *
 * Name:   ql_uart_register_txfwctrl_ex   
 *
 * Description: Register TX flow control callback
 *
 * Parameters:  port:Register the port for tx flow control callback
 *              
 * Returns:     none
 *              
 * Notes:
...
void ql_uart_register_txfwctrl(QL_UART_PORT_NUMBER_E port, uart_tx_fwctrl_callback uart_cb);
...
/*****************************************************************
* Function:     ql_uart_pin_set_func
* 
* Description:
*               用于设置uart pin脚的复用功能,方便客户自行配置uart pin.
*
* Parameters:
*               port:
*               [in]   uart port.
...
int ql_uart_pin_set_func(QL_UART_PORT_NUMBER_E port,       UINT32 PIN_NUM,unsigned int func_num);
/*****************************************************************
* Function:     ql_uart_pin_get_func
* 
* Description:
*               获取uart pin脚的复用功能号.
*
...
int ql_uart_pin_get_func(      UINT32 PIN_NUM);

int ql_uart_wakeup_mode_enable(QL_UART_PORT_NUMBER_E port);

int ql_uart_wakeup_mode_disable(QL_UART_PORT_NUMBER_E port);
