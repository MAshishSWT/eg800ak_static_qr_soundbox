...
/*****************************************************************
* Function: ql_sim_open_logical_channel
*
* Description:
* 	 该命令用于开启(U)SIM卡的逻辑信道(AT+CCHO)
* 
* Parameters:
* 	dfName	  		[in] 	字符串类型。UICC 中的所有可选应用程序，以1 到16 字节方式编码的DF 名称来引用。
* 	sessionId	  	[out] 	UINT16*类型。智能卡上的目标应用程序使用逻辑信道的会话ID。
*
* Return:
* 	QL_SIM_SUCCESS				Operation success.
*	QL_SIM_GENERIC_FAILURE		Operation failed.
*
*****************************************************************/
QL_SIM_ERROR_CODE ql_sim_open_logical_channel(char *dfName, UINT16* sessionId);

/*****************************************************************
* Function: ql_sim_close_logical_channel
*
* Description:
* 	 该命令使用指定的<sessionID>关闭(U)SIM卡的逻辑信道(AT+CCHC)
* 
* Parameters:
* 	sessionId	  		[in] 	整型。智能卡上的目标应用程序使用逻辑信道的会话ID。
*
* Return:
* 	QL_SIM_SUCCESS				Operation success.
*	QL_SIM_GENERIC_FAILURE		Operation failed.
...
*
*****************************************************************/
QL_SIM_ERROR_CODE ql_sim_close_logical_channel(UINT16 sessionId);

/*****************************************************************
* Function: ql_sim_access_logical_channel
*
* Description:
* 	 该命令用于访问UICC逻辑信道(AT+CGLA)
* 
* Parameters:
* 	sessionId	  		[in] 	整型。会话ID。用于将APDU 命令发送到UICC 的会话的标识符。智能卡上的目标应用程序使用逻辑信道而非默认信道（信道"0"）时，必须输入该参数向UICC 发送命令。
* 	commandlength	  	[in] 	整型。在 <command> 中发送给TE的字符长度（是命令或响应实际长度的两倍）。
* 	command 	  		[in] 	十六进制字符格式。MT 传递给UICC 的命令。详情可参考3GPP TS 31.101中的 AT+CSCS。
* 	resplength	  		[out] 	UINT16*类型。在<response> 中发送给TE的字符长度（是命令或响应实际长度的两倍）。 
* 	response	  		[out] 	十六进制字符格式。UICC 给MT 传递的命令的响应。详情可参考3GPP TS 31.101中的 AT+CSCS。
* 	
* Return:
* 	QL_SIM_SUCCESS				Operation success.
*	QL_SIM_GENERIC_FAILURE		Operation failed.
...
*
*****************************************************************/
QL_SIM_ERROR_CODE ql_sim_access_logical_channel(UINT16 sessionId, UINT16 commandlength, char *command, UINT16* resplength, char *response);
