...
#define QUEC_MAX_BLACK_CELL_NUM 10
#define QUEC_MAX_BLACK_CELL_LEN 128
#define QUEC_INVALID_AFRCN  0
#define QUEC_INVALID_CELLID 0
#define QUEC_BLACKCELL_ENABLE 1
#define QUEC_BLACKCELL_DISABLE 0
#define QUEC_BLACKCELLCFGNUM_MIN 1
#define QUEC_BLACKCELLCFGNUM_MAX 8
#define QUEC_BLACK_ELECTRIC_FENCE_EVENT_MIN 0
#define QUEC_BLACK_ELECTRIC_FENCE_EVENT_MAX 7


typedef enum{
    QUEC_AS_EVENT_LTE_RESELECTION_SUCCESS, //LTE重选小区成功
    QUEC_AS_EVENT_LTE_RESELECTION_FAIL, //LTE重选小区失败
    QUEC_AS_EVENT_LTE_HANDOVER_SUCCESS, //LTE切换成功
    QUEC_AS_EVENT_LTE_INTRA_HANDOVER_SUCCESS,   //LTE切换INTRA小区成功
    QUEC_AS_EVENT_LTE_INTRA_HANDOVER_FAIL,  //LTE切换INTRA小区失败
    QUEC_AS_EVENT_LTE_INTER_HANDOVER_SUCCESS,   //LTE切换INTER小区成功
    QUEC_AS_EVENT_LTE_INTER_HANDOVER_FAIL,  //LTE切换INTER小区失败
    QUEC_AS_EVENT_LTE_INTER_BAND_HANDOVER_SUCCESS,  //LTE切换BAND成功
    QUEC_AS_EVENT_LTE_INTER_BAND_HANDOVER_FAIL, //LTE切换BAND失败
    QUEC_AS_EVENT_LTE_REDIRECTION_SUCCESS,  //LTE重定向
    QUEC_AS_EVENT_LTE_MAX,
}QL_AS_EVNET_E;
    
typedef void (*QL_AS_EVENT_CB) (QL_AS_EVNET_E event);


/*****************************************************************
* Function: ql_nw_as_event_cb_register
*
* Description:
* 	注册用于接受AS层事件的回调函数。
* 
* Parameters:
* 	event_cb	  		[in] 	回调函数，当有QL_AS_EVNET_E内的事件时回调。 
*
* Return:
* 	QL_NW_SUCCESS			成功。
...
*
*****************************************************************/
QL_NW_ERROR_CODE ql_nw_as_event_cb_register(QL_AS_EVENT_CB event_cb);
...
/*****************************************************************
* Function: ql_nw_set_relev_min_offset
*
* Description:
* 	设置LTE S准则 q_RxLev 偏移,使用该接口前提必须在开机初始化时调用
* 
* Parameters:
* 	offset	  		[in] 	偏移数值
*
...
*
*****************************************************************/
QL_NW_ERROR_CODE ql_nw_set_relev_min_offset(int offset);

/*****************************************************************
* Function: ql_nw_get_relev_min_offset
*
* Description:
* 	获取LTE S准则 q_RxLev 偏移
* 
* Parameters:
* 	offset	  		[out] 	偏移数值
*
...
QL_NW_ERROR_CODE ql_nw_get_relev_min_offset(int *offset);
...
/*****************************************************************
* Function: ql_nw_get_band
*
* Description:
* 	获取csq信号强度
* 
* Parameters:
* 	band	  			[out] 	返回当前驻留的小区的 band 值。 
*
* Return:
* 	QL_NW_SUCCESS			成功。
*	QL_NW_GENERIC_FAILURE 	失败。
*
*****************************************************************/
QL_NW_ERROR_CODE ql_nw_get_band(int *band);


/*****************************************************************
* Function: QL_NW_ERROR_CODE ql_nw_get_sinr(int *sinr)
*
* Description:
* 	获取当前小区的sinr值
* 
* Parameters:
* 	sinr	  			[out] 	返回当前小区的 sinr 值。 
*
* Return:
* 	QL_NW_SUCCESS			成功。
*	  QL_NW_GENERIC_FAILURE 	失败。
*
*****************************************************************/
QL_NW_ERROR_CODE ql_nw_get_sinr(int *sinr);
...
* 
* Parameters:
* 	option                   [in]   扫描超时时间，4000-255000ms，默认是12秒，单位是毫秒
...
*
*****************************************************************/
QL_NW_ERROR_CODE ql_nw_get_wifiscan_info(unsigned long option, unsigned short scanRoundNum, unsigned short scanMaxBssidNum, unsigned short sacnTimeout, unsigned short scanPriority, unsigned char sendUrcFlag);
...
/*****************************************************************
* Function: ql_delete_all_blackcell
*
* Description:
* 	清除异常基站策略中所有黑名单小区
* 
...
*
*****************************************************************/
QL_NW_ERROR_CODE ql_delete_all_blackcell(void);

/*****************************************************************
* Function: ql_set_black_cell_cfg
*
* Description:
* 	使能及配置异常基站策略(old) 
* Parameters:
* 	blackcellCfg              [in]   整型。0/1 使能/关闭异常基站策略(old)。默认配置 0
*	blackcellCfg_num          [in]   整型。异常基站策略 坏事件触发次数
* Return:
* 	QL_NW_SUCCESS			成功。
*	QL_NW_GENERIC_FAILURE 	失败。
*
...
QL_NW_ERROR_CODE ql_set_black_cell_cfg(int blackcellCfg,int blackcellCfg_num);
/*****************************************************************
* Function: ql_get_black_cell_cfg
*
* Description:
* 	查询异常基站策略(old)
* 
* Parameters:
* 	blackcellCfg              [out]   整型。0/1 使能/关闭异常基站策略(old)。默认配置 0
*	blackcellCfg_num          [out]   整型。异常基站策略 坏事件触发次数
* Return:
* 	QL_NW_SUCCESS			成功。
*	QL_NW_GENERIC_FAILURE 	失败。
*
*****************************************************************/
QL_NW_ERROR_CODE ql_get_black_cell_cfg(int* blackcellCfg,int* blackcellCfg_num);


/*****************************************************************
* Function: ql_set_black_cell_cfg_ex
*
* Description:
* 	使能及配置异常基站策略
* 
* Parameters:
* 	blackcellCfg              [in]   整型。0/1 使能/关闭异常基站策略。默认配置 0
*	delete_timer_size         [in]   整型。以定时清除 拉黑小区时的定时器长度。范围：300~86400；单位：秒，默认配置1800
*	prevention_mode           [in]   整型。0/1 使能/关闭防呆机制。默认配置 1
*   prevention_timer_size     [in]   整型。无法注册时，按权重逐级释放黑名单小区的定时器长度 ，范围60-3600，单位：秒，默认配置300。（首次触发后减半）
*   reject_judge_mode         [in]   整型。0/1 异常场景1-3的计数判断机制，默认配置0。
*   reject_judge_count        [in]   整型。异常场景1-3的计数门限，范围1-255   默认配置5（场景2门限对5向上取整）
*   black_loop_num            [in]   整型。异常场景5频繁重选判定的门限次数，范围1-255 默认配置3
*   black_loop_cycle          [in]   整型。异常场景5频繁重选判定的门限时间，范围60-86400                       默认配置600
* Return:
* 	QL_NW_SUCCESS			成功。
*	QL_NW_GENERIC_FAILURE 	失败。
*
*****************************************************************/
QL_NW_ERROR_CODE ql_set_black_cell_cfg_ex(
						int blackcellCfg,
						UINT32 delete_timer_size,
						int prevention_mode,
						UINT32 prevention_timer_size,
						int reject_judge_mode,
						int reject_judge_count,
						int black_loop_num,
						UINT32 black_loop_cycle);

/*****************************************************************
* Function: ql_get_black_cell_cfg_ex
*
* Description:
*	查询异常基站策略
* 
* Parameters:
* 	blackcellCfg             [out]   整型。0/1 使能/关闭异常基站策略。默认配置 0
*	delete_timer_size        [out]   整型。以定时清除 拉黑小区时的定时器长度。范围：300~86400；单位：秒，默认配置1800
*	prevention_mode          [out]   整型。0/1 使能/关闭防呆机制。默认配置 1
*   prevention_timer_size    [out]   整型。无法注册时，按权重逐级释放黑名单小区的定时器长度 ，范围60-3600，单位：秒，默认配置300。（首次触发后减半）
*   reject_judge_mode        [out]   整型。0/1 异常场景1-3的计数判断机制，默认配置0。
*   reject_judge_count       [out]   整型。异常场景1-3的计数门限，范围1-255   默认配置5（场景2门限对5向上取整）
*   black_loop_num           [out]   整型。异常场景5频繁重选判定的门限次数，范围1-255 默认配置3
*   black_loop_cycle         [out]   整型。异常场景5频繁重选判定的门限时间，范围60-86400                       默认配置600
* Return:
*	QL_NW_SUCCESS			成功。
*	QL_NW_GENERIC_FAILURE	失败。
*
*****************************************************************/
QL_NW_ERROR_CODE ql_get_black_cell_cfg_ex(
						int* blackcellCfg,
						UINT32* delete_timer_size,
						int* prevention_mode,
						UINT32* prevention_timer_size,
...
/*****************************************************************
* Function: ql_set_black_event_weight_ex
*
* Description:
*	配置异常基站策略相关场景权重
* 
* Parameters:
*	event_index			     [in]	整型。0-7 异常基站策略场景索引
*	blackeventweight 		 [in]	整型。0-255 场景异常事件对应黑名单权重
*
* Return:
*	QL_NW_SUCCESS			成功。
*	QL_NW_GENERIC_FAILURE	失败。
*
*****************************************************************/
QL_NW_ERROR_CODE ql_set_black_event_weight_ex(int event_index,int blackeventweight);


/*****************************************************************
* Function: ql_get_black_event_weight_ex
*
* Description:
*	查询异常基站策略相关场景权重
* 
* Parameters:
*	event_index			     [out]	整型。0-7 异常基站策略场景索引
*	blackeventweight 		 [out]	整型。0-255 场景异常事件对应黑名单权重
*
* Return:
*	QL_NW_SUCCESS			成功。
*	QL_NW_GENERIC_FAILURE	失败。
*
*****************************************************************/
QL_NW_ERROR_CODE ql_get_black_event_weight_ex(int event_index,int* blackeventweight);


/*****************************************************************
* Function: ql_set_black_lwip_cfg_ex
*
* Description:
*	使能及配置异常基站策略 数据不通检测场景
* 
* Parameters:
*	blacklwipcfg_vaild			 [in]	整型。0/1 使能/关闭异常基站策略 数据不通检测。默认配置 0
*	blacklwip_fencenum 		     [in]	整型。异常基站策略 数据不通判定的门限包数。范围：1~255；单位：包，默认配置100
*	blacklwip_fencecycle		 [in]	整型。异常基站策略 数据不通判定的门限时间。范围：60-86400  默认配置600 
*	blacklwip_retrynum 		     [in]	整型。判定数据不通且ping预置host失败时，尝试CFUN0/1的次数，默认配置3
*	blackping_host				 [in]	整型。字符串类型。预置host，判定数据不通时ping预置host，检测终端网络状态
*	blackcell_vaild				 [in]	整型。整型。是否基于检测结果拉黑小区(暂不支持)
* Return:
*	QL_NW_SUCCESS			成功。
*	QL_NW_GENERIC_FAILURE	失败。
*
*****************************************************************/
QL_NW_ERROR_CODE ql_set_black_lwip_cfg_ex(
						int blacklwipcfg_vaild,
						int blacklwip_fencenum,
						int blacklwip_fencecycle,
						int blacklwip_retrynum,
						char* blackping_host,
						int blackcell_vaild);


/*****************************************************************
* Function: ql_get_black_lwip_cfg_ex
*
* Description:
*	查询异常基站策略-数据不通检测场景配置
* 
* Parameters:
*	blacklwipcfg_vaild			 [out]	整型。0/1 使能/关闭异常基站策略 数据不通检测。默认配置 0
*	blacklwip_fencenum 		     [out]	整型。异常基站策略 数据不通判定的门限包数。范围：1~255；单位：包，默认配置100
*	blacklwip_fencecycle		 [out]	整型。异常基站策略 数据不通判定的门限时间。范围：60-86400  默认配置600 
*	blacklwip_retrynum 		     [out]	整型。判定数据不通且ping预置host失败时，尝试CFUN0/1的次数，默认配置3
*	blackping_host				 [out]	整型。字符串类型。预置host，判定数据不通时ping预置host，检测终端网络状态
*	blackcell_vaild				 [out]	整型。整型。是否基于检测结果拉黑小区(暂不支持)
* Return:
*	QL_NW_SUCCESS			成功。
*	QL_NW_GENERIC_FAILURE	失败。
*
*****************************************************************/
QL_NW_ERROR_CODE ql_get_black_lwip_cfg_ex(
						int* blacklwipcfg_vaild,
						int* blacklwip_fencenum,
						int* blacklwip_fencecycle,
						int* blacklwip_retrynum,
...
/*****************************************************************
* Function: ql_nw_auto_apn_enable
*
* Description:
*   使能auto apn功能
* 
* Parameters:
*   enable           [in]   整型。0/1 关闭/开启。默认配置 1
* Return:
*   QL_NW_SUCCESS           成功。
*   其他值             失败。
*
*****************************************************************/
QL_NW_ERROR_CODE ql_nw_auto_apn_enable(BOOL enable);
...
*
*****************************************************************/
int ql_nw_auto_apn_is_enable(void);
